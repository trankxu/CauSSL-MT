...
def epochVal(model, dataLoader, loss_fn, args):
    ...
    with torch.no_grad():
        for i, (study, image, label) in enumerate(dataLoader):
            ...
            mask_part, non_mask_part, non_mask_out, mask_out, _t = model(image)
            # _, output = model(image)

            loss = loss_fn(mask_out, label.clone())
...
def epochVal_metrics_test(model, dataLoader):
    ...
    with torch.no_grad():
        for i, (study, _, image, label) in enumerate(dataLoader):
            image, label = image.cuda(), label.cuda()
            mask_part, non_mask_part, non_mask_out, mask_out, _ = model(image)
            output = F.softmax(mask_out, dim=1)
            # _, output = model(image)
