...
parser.add_argument('--pre_epochs', type=int, default=0, help='')
...
if __name__ == "__main__":
    ...
    def create_model(ema=False):
        ...
        model = net.cuda()#to("cuda:"+args.gpu)
        if ema:
    ...
    pre_train_dataloader = DataLoader(dataset=pre_train_dataset, batch_size=64,  # sampler=pre_train_sampler,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch < args.pre_epochs:
            for i, (_, _, (image_batch, ema_image_batch),_) in enumerate(pre_train_dataloader):
                ...
                image_batch, ema_image_batch = image_batch.to("cuda:"+args.gpu), ema_image_batch.to("cuda:"+args.gpu)

                inputs = image_batch
                ema_inputs = ema_image_batch

                #activations, outputs = pre_model(inputs)
                non_mask_part,mask_part, non_mask_out, mask_out, activations = pre_model(inputs)
                with torch.no_grad():
                    #ema_activations, ema_output = pre_ema_model(ema_inputs)
                    ema_non_mask_part, ema_mask_part, ema_non_mask_out, ema_mask_out, ema_activations = pre_ema_model(ema_inputs)
                ## calculate the loss
            ...
            for i, (_,_, (image_batch, ema_image_batch), label_batch) in enumerate(train_dataloader):
                ...
                # print('fetch data cost {}'.format(time2-time1))
                image_batch, ema_image_batch, label_batch = image_batch.to("cuda:"+args.gpu), ema_image_batch.to("cuda:"+args.gpu), label_batch.to("cuda:"+args.gpu)
                ...
                non_mask_part, mask_part, non_mask_out, mask_out, activations = meta_net(inputs)
                ...
                with torch.no_grad():
                    ...
                    ema_output_detach = ema_mask_out.detach()

                weight = wnet(mask_out.softmax(1)[labeled_bs:])
                ...
                loss = loss_classification #+ mask_loss
                ...
                if args.ema_consistency == 1:
                    ...
                    consistency_dist_l = torch.sum(losses.softmax_mse_loss(mask_out[:labeled_bs], ema_mask_out[:labeled_bs])) / labeled_bs  # / dataset.N_CLASSES
                    consistency_dist_ul = torch.sum(weight * losses.softmax_mse_loss(mask_out[labeled_bs:], ema_mask_out[labeled_bs:])) / norm_weight  # / dataset.N_CLASSES
                ...
                if  (args.ema_consistency == 1):#
                    loss = loss_classification + contrastive_loss + consistency_loss + mask_loss
 
                meta_net.zero_grad()
                #for param in meta_net.params():
                    #param.requires_grad = True
                grads = torch.autograd.grad(loss, (meta_net.params()), create_graph=True, allow_unused=True)
                #from torchviz import make_dot
                #dot = make_dot(grads, params=dict(meta_net.named_parameters()))
                #dot.render("computation_graph", format="pdf")
                '''
                for (name, param), grad in zip(meta_net.named_parameters(), grads):
                    if grad is None:
                        print("Gradient is None for parameter:",name)#,param)
                '''  
                meta_net.update_params(lr_inner=args.meta_lr, source_params=grads)
                del grads

                #_, y_g_hat = meta_net(inputs)
                meta_non_mask_part, meta_mask_part, meta_non_mask_out, meta_mask_out, meta_activations = meta_net(inputs)
                l_g_meta = loss_fn(meta_mask_out[:labeled_bs], label_batch[:labeled_bs])
                ...
                mask_loss = loss_fn(rand_var, non_mask_out)
                loss = loss_classification + mask_loss
